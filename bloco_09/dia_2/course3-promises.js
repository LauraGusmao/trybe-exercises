...
// Para criar a Promise, utilizamos o construtor Promise junto com o operador new. Um construtor nada mais é do que uma função especial que cria um objeto a partir de uma classe. Não preocupe com classes ou cronstrutores no momento, apenas observe a sintaxe.
// O construtor recebe uma função com 2 parâmetros: resolve e reject. Um ponto a se notar aqui é que usamos uma arrow function para passar a função como parâmetro pro construtor, mas você pode utilizar qualquer construção, desde que seja uma função com 2 parâmetros.
...
// Quando o programa é bem sucedido, seu output aparece normalmente. Já quando ele é mal sucedido, ele lança um error! Essa é a principal diferença entre o resolve e o reject, exatamente o que queríamos demonstrar. Na próxima sessão vamos ver como tratar este erro.
...
// Vamos criar uma promise que faz um fetch apenas para um endpoint específico. Para isso, vamos usar como "endpoint" uma url que nos retorna elogios sobre Chuck Norris.

// Antes de irmos para o código, como vamos rodar nosso código direto no Node e não no browser, precisamos instalar o node-fetch. Execute o comando "npm init -y" caso você ainda não tenha o package.json. Depois instale o node-fetch: "npm i node-fetch".
...
// Ao executarmos a função sendJokeToFriend recebemos o seguinte log: Promise { <pending> }.
